from tensorflow.keras.layers import Lambda, Activation, Dense, Conv2D, Conv2DTranspose, Input, BatchNormalization, Add, MaxPool2D
...
"""
LINKNET Model
Paper: https://ieeexplore.ieee.org/abstract/document/8305148
# ----------------------------------------------------------------------------------------------
"""
...
def linknet(config):
    ...
    def encoder_block(input_tensor, block=None):
        ...
        input_tensor_projection = Conv2D(filters=nfilters, kernel_size=(1, 1), strides=(2, 2), padding='same', kernel_initializer='he_normal', name=f'{block_name}_conv2d_1x1')(input_tensor)
        input_tensor_projection = BatchNormalization(name=f'{block_name}_bn_1x1')(input_tensor_projection)
        input_tensor_projection = Activation('relu', name=f'{block_name}_relu_1x1')(input_tensor_projection)

        x = Conv2D(filters=nfilters, kernel_size=(3, 3), strides=(2, 2), padding='same', kernel_initializer='he_normal', name=f'encoder_{block_name}_conv2d_1')(input_tensor)
        x = BatchNormalization(name=f'encoder_{block_name}_bn_1')(x)
        x = Activation('relu', name=f'encoder_{block_name}_relu_1')(x)
        x = Conv2D(filters=nfilters, kernel_size=(3, 3), strides=(1, 1), padding='same', kernel_initializer='he_normal', name=f'encoder_{block_name}_conv2d_2')(x)
        x = BatchNormalization(name=f'encoder_{block_name}_bn_2')(x)
        x = Add(name=f'encoder_{block_name}_add_1')([x, input_tensor_projection])
        x = Activation('relu', name=f'encoder_{block_name}_relu_2')(x)
        x_res = x

        x = Conv2D(filters=nfilters, kernel_size=(3, 3), strides=(1, 1), padding='same', kernel_initializer='he_normal', name=f'encoder_{block_name}_conv2d_3')(x_res)
        x = BatchNormalization(name=f'encoder_{block_name}_bn_3')(x)
        x = Activation('relu', name=f'encoder_{block_name}_relu_3')(x)
        x = Conv2D(filters=nfilters, kernel_size=(3, 3), strides=(1, 1), padding='same', kernel_initializer='he_normal', name=f'encoder_{block_name}_conv2d_4')(x)
    ...
    def decoder_block(input_tensor, block=None):
        ...
        x = Conv2D(filters=nfilters_a, kernel_size=(1, 1), strides=(1, 1), padding='same', kernel_initializer='he_normal', name=f'decoder_{block_name}_conv2d_1x1')(input_tensor)
        x = BatchNormalization(name=f'decoder_{block_name}_bn_1')(x)
        x = Activation('relu', name=f'decoder_{block_name}_relu_1')(x)
        x = Conv2DTranspose(filters=nfilters_a, kernel_size=(3, 3), strides=(2, 2), padding='same', kernel_initializer='he_normal', name=f'decoder_{block_name}_conv2dT_1')(x)
        x = BatchNormalization(name=f'decoder_{block_name}_bn_2')(x)
        x = Activation('relu', name=f'decoder_{block_name}_relu_2')(x)
        x = Conv2D(filters=nfilters_b, kernel_size=(1, 1), strides=(1, 1), padding='same', kernel_initializer='he_normal', name=f'decoder_{block_name}_conv2d_1x2')(x)
    ...
    y = Conv2DTranspose(filters=32, kernel_size=(3, 3), strides=(
        2, 2), padding='same', kernel_initializer='he_normal', name='final_conv2dT')(y)
    y = BatchNormalization(name='final_block_bn_1')(y)
    y = Activation('relu', name='final_block_relu_1')(y)
    y = Conv2D(filters=32, kernel_size=(3, 3), strides=(
